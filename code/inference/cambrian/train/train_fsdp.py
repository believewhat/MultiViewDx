...
wandb.login(key="7d981310ad7a3b15717e421fc684775df401eb6c")
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_length(self):
        ...
        return i + 1
        """
        return len(json.load(open(self.data_path, "r")))
    ...
    def _compute_lengths(self):
        ...
        for sample in json.load(open(self.data_path, "r")):
        ...
        """
        for line in self.list_data_dict:
            sample = line
            img_tokens = self.data_args.image_token_len if self._has_image(sample) else 0
            cur_len = sum(len(conv['value'].split()) for conv in sample['conversations'])
            self.length_list.append(cur_len + img_tokens)
        ...
        with open(self.data_path, 'r') as file:
            for idx, line in enumerate(file):
                if idx == i:
                    ...
                    break
        """
        with open(os.path.join(self.data_path2, f"{i}.json"), 'r') as file:
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = LazySupervisedDataset(tokenizer=tokenizer,
                                ...
                                data_path2=data_args.data_path2,
                                data_args=data_args)
