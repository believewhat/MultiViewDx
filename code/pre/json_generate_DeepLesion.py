...
def extract_questions_answers_with_image_path(content1, content, txt_path, image_base_dir):
    """
    ...
    relative_path = txt_dirname.split('instruct', 1)[1]  # Extract the relative path
...
def process_directory(base_dir, image_base_dir):
        ...
        for file in files:
            if file.endswith(".txt"):
                ...
                with open(txt_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                txt_path2 = txt_path.replace('instruct', 'json')
                try:
                ...
                conversations, image_path = extract_questions_answers_with_image_path(content1, content, txt_path, image_base_dir)
    ...
    with open('DeepLesion.json', 'w', encoding='utf-8') as json_file:
        json.dump(all_conversations, json_file, indent=4, ensure_ascii=False)

    print(f"Conversations extracted and saved to 'DeepLesion.json'.")

# Specify base directories and run the process
base_dir = '/mnt/cache_share/MedTrinity-25M/DeepLesion/instruct'
image_base_dir = '/mnt/cache_share/MedTrinity-25M/DeepLesion/Key_slices'
