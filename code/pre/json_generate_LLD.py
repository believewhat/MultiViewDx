...
def extract_questions_answers_with_image_path(content, txt_path, image_base_dir):
    ...
    # Improved regex to handle variations including **Report**, _Report_, #### Report, etc.
    report_match = re.search(r"(?i)[*_#]{0,4}Report[*_#]{0,4}:\s*(.*?)\n+(?=\s*[*_#]{0,4}Questions?[*_#]{0,4}:|\Z)", content, re.DOTALL | re.IGNORECASE)
    ...
    if report_match:
        ...
        print(f"未找到 Report 部分在文件: {txt_path}")
    
    # Extracting the question and answer blocks, handling **Question**, _Question_, #### Question, etc.
    ...
    for root, dirs, files in os.walk(base_dir):
        for file in files:
            if file.endswith(".txt"):
                ...
                # Extract conversations and generate image path
                conversations, image_path = extract_questions_answers_with_image_path(content, txt_path, image_base_dir)

                if conversations and image_path:  # Ensure valid conversations and path
                    all_conversations.append({
                        "conversations": conversations,
                        "image": image_base_dir + image_path
                    })

    # Save the extracted conversations to a JSON file
    with open('LLD.json', 'w', encoding='utf-8') as json_file:
        json.dump(all_conversations, json_file, indent=4, ensure_ascii=False)

    print(f"Conversations extracted and saved to 'LLD.json'.")

# Specify base directories and run the process
base_dir = '/mnt/cache_share/MedTrinity-25M/LLD-MMRI/json'
image_base_dir = '/mnt/cache_share/MedTrinity-25M/LLD-MMRI/images2'
